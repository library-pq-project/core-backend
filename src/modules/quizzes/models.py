...
class QuizResponse(Base):
    ...
    answer_input_mode: Mapped[str] = mapped_column(String(20), nullable=False, default="typed", server_default="typed")
    answer_file_provider: Mapped[str | None] = mapped_column(String(30), nullable=True)
    answer_file_bucket: Mapped[str | None] = mapped_column(String(120), nullable=True)
    answer_file_key: Mapped[str | None] = mapped_column(String(255), nullable=True)
    answer_file_path: Mapped[str | None] = mapped_column(String(500), nullable=True)
    answer_file_type: Mapped[str | None] = mapped_column(String(20), nullable=True)
    answer_file_size: Mapped[int | None] = mapped_column(Integer, nullable=True)
    answer_extracted_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    answer_extraction_status: Mapped[str] = mapped_column(String(30), nullable=False, default="pending", server_default="pending")
    is_correct: Mapped[bool | None] = mapped_column(nullable=True)
    score_awarded: Mapped[float | None] = mapped_column(Numeric(8, 2), nullable=True)
    feedback: Mapped[str | None] = mapped_column(Text, nullable=True)
    grading_strengths: Mapped[str | None] = mapped_column(Text, nullable=True)
    grading_missing_points: Mapped[str | None] = mapped_column(Text, nullable=True)
    grading_confidence: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
    grading_explanation: Mapped[str | None] = mapped_column(Text, nullable=True)
